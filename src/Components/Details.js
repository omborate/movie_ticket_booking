...
function Details() {
  ...
  const handleSubmit = async (e) => {
    ...
    console.log(gender1);
    console.log(seatValue);
    let records = await httpClient.post("http://localhost:5000/add", {
      name1,
      age1,
      gender1,
      seatValue,
  ...
  const handlePatch = () => {
    const patchData = {
      id: seatValue,
      patchOperation: [{ op: "replace", path: "/value", value: formData }],
    };

    axios
      .patch(
        `http://localhost:5000/update/${patchData.id}`,
        patchData.patchOperation
      )
      .then((response) => {
